...
动态规划（Dynamic Programming，简称 DP）是一种在计算机科学和数学中用来求解最优化问题的方法。它通常用于解决那些具有重叠子问题（即同一个子问题会被多次求解）和最优子结构（即问题的最优解可以通过其子问题的最优解来构造）的问题。
...
假设我们要解决经典的背包问题，其中每个物品有重量和价值，我们需要选择一些物品放入背包中使得总价值最大但不超过背包的最大承重。

- **状态定义**：设 `dp[i][w]` 表示前 `i` 个物品中选择一些装入容量为 `w` 的背包可以获得的最大价值。
- **状态转移方程**：`dp[i][w] = max(dp[i-1][w], dp[i-1][w-weight[i]] + value[i])`，其中 `weight[i]` 和 `value[i]` 分别是第 `i` 个物品的重量和价值。
- **边界条件**：`dp[0][w] = 0`（没有物品时价值为 0），`dp[i][0] = 0`（背包容量为 0 时价值也为 0）。
- **计算顺序**：从 `i=1, w=1` 开始，按行或列的顺序依次填充表格。
- **存储结果**：最终答案位于 `dp[n][W]`，其中 `n` 是物品总数，`W` 是背包的最大承重。

在实际应用中，具体的状态定义、状态转移方程以及计算顺序可能会有所不同，取决于具体问题的特点。
...
int main() {
    ...
    // 创建一个二维向量来存储物品的重量和价值
    vector<vector<int>> items(num_items, vector<int>(2));
    cout << "请输入每个物品的重量和价值(用空格分隔):" << endl;
    for (int i = 0; i < num_items; ++i) {
        cin >> items[i][0] >> items[i][1]; // 第一列是重量，第二列是价值
    }
    ...
    for (int i = 1; i <= num_items; ++i) {
        for (int w = 1; w <= capacity; ++w) {
            if (items[i - 1][0] <= w) {
                // 如果当前物品可以放入背包，则比较放与不放两种情况的价值
                dp[i][w] = max(dp[i - 1][w], dp[i - 1][w - items[i - 1][0]] + items[i - 1][1]);
            } else {
                // 如果当前物品不能放入背包，则直接继承上一个物品的价值
                dp[i][w] = dp[i - 1][w];
            }
        }
    }
    ...
    // 输出最大价值
    cout << "最大价值为: " << dp[num_items][capacity] << endl;
