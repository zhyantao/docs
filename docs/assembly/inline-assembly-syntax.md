...
上面这样写，很容易让你的程序崩掉。
因为 GCC 不知道这几个 `asm` 语句修改了 `ebx`、`edx` 等寄存器，
它可能仍然假定这些寄存器里保存着原来的值，从而产生错误的结果，而这个原因是 GCC 不会告诉你的。

那更好的方式应该是用下面介绍的格式来进行书写：不特别指定寄存器，让 GCC 自动优化，决定使用哪个寄存器。
我们使用 `%0`、`%1`、`%2` 这种方式来让程序更有弹性。
...
`"q"` 和 `"r"` 从哪里来？`"r"` 让 GCC 可以从任意通用寄存器
（`eax`、`ebx`、`ecx`、`edx`、`esi`、`edi`）中做选择，而 `"q"` 将选择范围限制在
`eax`、`ebx`、`ecx` 和 `edx`（即那些可以按字节寻址的寄存器）中。所以，除非你需要硬编码特定的
寄存器（如 `rep movsl` 要求使用 `ecx`、`esi`、`edi`），一般情况下建议使用 `"r"`，让 GCC 有更大的优化空间。
