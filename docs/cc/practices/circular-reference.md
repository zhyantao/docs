...
当两个对象需要紧密协作、但**不存在所有权关系**时，就不应该用 `shared_ptr` 互相持有对方
（那正是形成引用环的根源），而应该使用普通引用/指针临时访问。下面的例子中 `Sniper` 与
`Supplier` 并不互相持有对方，`Supplier::provide()` 只是通过友元机制访问 `Sniper` 的私有成员：

```{note}
严格来说，这个例子中并没有形成引用环——它演示的是第三种思路：**从设计上避免共享所有权**。
没有共享所有权，循环引用问题就根本不会产生。
```
