...
```{warning}
`const_cast` 只是移除了常量性限定，并不改变对象本身的常量属性。如果原始对象本身就是
`const`（如上面的 `ci`），通过 `pi` 修改它是**未定义行为**。`const_cast` 的典型合法用途是：
把本来非常量、但以 `const` 引用/指针传入函数的对象"恢复"为可写。
```

## static_cast

`static_cast` 是一种更安全的、基于编译期类型信息的类型转换。它可以完成大多数传统的类型转换操作，例如数值类型之间的转换（int/double）、从派生类到基类的上行转换、`void*` 与其他指针之间的转换等。它比 C 风格的类型转换更安全，因为它会在编译时检查类型是否兼容。

```{note}
从整数到指针的转换（如 `static_cast<int*>(0x1234)`）不能使用 `static_cast`（编译错误，
0 这类空指针常量除外），必须使用 `reinterpret_cast`。
```
...
- 对非多态类型（不含虚函数的类）使用 `dynamic_cast` 会导致**编译错误**，而不是运行时返回空指针。
