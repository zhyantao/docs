...
我们经常混淆数据库和数据库管理系统的概念，那么 MySQL 是一个数据库呢，还是一个数据库管理系统？
...
按照数据模型的不同，数据库管理系统可以分为关系型数据库管理系统和非关系型数据库管理系统。
MySQL 可以作为关系型数据库管理系统的代表。

## 安装 MySQL

安装 MySQL 可能会遇到一些坑，不推荐使用 Installer 安装，它会装很多不需要的环境。
...
{numref}`mysql-arch` 展示了当客户端发出一条指令后，MySQL 将做出的一系列响应 [^cite_ref-2]。

```{figure} ../../_static/images/db_mysql_architecture.*
:name: mysql-arch
...
数据库管理系统对数据库的操作终究不是原语操作，难免导致数据不一致。
如何避免这些现象，或当现象发生后如何补救，就成了我们研究的重点。

考虑数据存储的载体数量和位置。如果数量较少、只有一块硬盘，那么事务的实现其实十分简单，但真实场景通常不会这样。
而对于多块数据盘，它们之间的通信是否流畅、是否会发生网络分区，也是应当着重考虑的问题。

如何保证事务？常用的手段就是日志，而日志的本质就是一个个存储在磁盘上的文件，
它们记录了你修改了哪些数据、改成了什么、原来是什么。

日志通常不只有一种，比如可以分为 Redo log、binlog、Undo log，在什么场合使用什么类型的 log
如何组合使用其中的某几种 log 来实现目标，也是我们应该熟知的。
