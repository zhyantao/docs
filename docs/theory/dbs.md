...
在关系数据库中，表中的每一行都代表一条记录，每条记录都具有一个唯一的 ID（又被称为键），而表中的列则用于存储数据的属性。
每条记录的每一个属性通常都有一个值。借此，用户可以轻松地在数据点之间建立关联 [^cite_ref-1]。

NoSQL 数据库（意即“不仅仅是 SQL”）并非表格格式，其存储数据的方式与关系表不同。
NoSQL 数据库的类型因数据模型而异，主要类型包括文档、键值、宽列和图形。
它们提供了灵活的模式，可以随大量数据和高用户负载而轻松扩展 [^cite_ref-2]。

关系型数据库的代表有：MySQL、Oracle、SQL Server、SQLite。

非关系型数据库的代表有：Redis、MongoDB、Neo4j、HBase。
...
* - 四种 SQL 语言 [^cite_ref-4]
  - 数据库定义语言（DDL，Data Definition Language）定义 SQL 模式、基本表、视图、索引等。
    ...
    事务控制语言（TCL，Transaction Control Language）设置保存点、回滚等。`SAVEPOINT`、`ROLLBACK`、`SET TRANSACTION`。
* - 数据模型
...
* - 除
  - $R \div S = \Pi_{R-S}(R)-\Pi_{R-S}((\Pi_{R-S}(R) \times S)-R)$
    ...
    注意，这里的 $R - S$ 指的是属性做差，前面讲的是元组做差。除法比较难理解，从结果往回看比较好理解。
...
要想回答这个问题，如果直接采用常规思路，将很难理解解题过程。
根据语义转化，我们可以把它描述为：在 S1 生产的_全部零件_中，**不存在**一个它**不**使用的零件。
因为这个语义中含有两个否定，因此需要两个 `not exist` 来完成从自然语言到 SQL 语言的转化。
...
这道题看过好几遍了，每次看都不是很理解，**现在也不是很理解**。所以说，`exists` 可真是个难点。
...
SQL 语言支持的完整性约束包括：静态约束（列完整性和表完整性）和动态约束（触发器）。

不知道你注意到没有，我们在创建表的时候，其实是有完整性约束的，从关键字上的体现就是
`primary key`、`not null`、`unique`。它们保证了列完整性约束（或域完整性）。
...
在关系数据库理论中，函数依赖（FD）是关系数据库中两个属性集合之间的一种约束。
...
我们的目标就是消除关系模式中的函数依赖。本节我们将介绍几个函数依赖的定义：
...
若单从定义上来理解保持依赖，具有一定的难度，但我们还是把定义写在这里：

设有关系模式 $R$，$F$ 是 $R$ 的函数依赖集，$Z$ 是 $R$ 的一个属性集合，则称 $Z$ 涉及的 $F^+$
...
既然已经理解了无损连接和保持依赖的求解方法，那么它们有什么指导意义呢？
...
关于数据库的物理存储，我们希望合理地组织数据，以实现高效存取。机械硬盘的物理结构如下：
...
我们知道，Windows 操作系统支持 NTFS、FAT32、exFAT 三种不同的{ref}`windows_file_system`。
文件系统是系统对文件的存放排列方式，不同格式的文件系统关系到数据如何在磁盘进行存储。

那么，MySQL 组织文件的方式与操作系统有何不同？

操作系统以文件的方式（FAT）组织数据和存取数据，MySQL 则以磁盘块为单位来组织数据和存取数据。
...
RAID 全称独立磁盘冗余阵列，是由许多价格便宜的磁盘组成一个容量巨大的磁盘组、以提升存储性能或提高数据安全的技术。
这项技术一方面可以提高存取性能，另一方面也提高了数据库的可靠性。
...
提供最高的数据安全保障，但磁盘利用率为 50%，故成本最高。

3、RAID 5（RAID 0 和 RAID 1 的折中方案）

```{figure} ../_static/images/db_raid_5.*
:height: 200px
```

没有完全采用 RAID 1 的镜像理念，而是使用“奇偶校验信息”作为数据恢复的方式，但这与下面的 RAID 10 不同。

4、RAID 10（RAID 0 和 RAID 1 的折中方案）
...
RAID 10 也被称为镜像阵列条带，提供最好的性能和更好的可靠性。

按照数据的类型来说，有记录、表、文件三类，对应的数据库组织方式可以有：
...
- 连续分配：数据块被分配到连续的磁盘块上（存在扩展困难的问题）
- 链接分配：数据块中包含指向下一数据块的指针（存在访问速度问题）
- 按簇分配：簇是若干连续的磁盘块，簇之间靠指针连接（簇有时候也称片段Segment或盘区extent）
- 索引分配：索引块中存放指向实际数据块的指针

对于一个文件，可选的组织方式：

- 堆文件（Heap 或 Pile file）：无序，检索效率差；删除文件后需要重新组织数据库
- 排序文件（Sequential）：有序，更新效率差；文件溢出后需要重新组织数据库
- 散列文件（Hash file）：散列到桶中，桶内按顺序检索，难点在于如何让每个桶尽量均匀
- 聚簇文件（Clustering file）：将具有相似或相同属性值的记录存放在连续的磁盘簇块中

根据文件的组织方式，我们可以对文件建立 B+ 树索引或散列索引。

综上，没有一个十分完美的方案，经常是在鱼和熊掌之间做取舍，因此，符合业务场景的就是合适的。

## 查询的实现

讲到查询的实现，不可避免地要涉及索引的概念，比如 InnoDB 的默认索引就是 B+ 树。
索引是定义在存储表基础上、无需检查所有记录即可快速定位所需记录的一种辅助存储结构，由一系列存储在磁盘上的索引项组成。
...
3. 常规索引 `key` / `index`：可以由多个列声明为常规索引。
4. 全文索引 `fulltext index`：并不是所有的引擎都支持全文索引，比如 MyISAM。
...
`Ki` 是索引字段值，`Pi` 是指针，指向索引块、数据块或数据块中记录的指针。
最后一个 `Pn` 指向下一个 B+ 树结点。
...
举例来说，操作分成两个步骤：先将主文件记录所在的磁盘块加载到内存中，然后采用循环的方式对比结果。
大体的伪代码如下所示：
...
当然，这是一个很朴素的想法，有很大的优化空间，想在这里讲明白也不是一件容易的事，后面有机会再补充。

## 查询的优化

所谓查询优化，指的是“如何使数据库查询的执行时间最短”。有三条启发式的优化原则供参考：
...
我们书写的每一条（一组）SQL 语句都可以看作一个事务。
多个看似并行的事务，实际上是交叉执行的（并发）。那么如何保证数据的一致性，就是本节需要讨论的话题。

为什么需要事务？在一个提供服务的程序中，事务可能会由于 CPU 调度在执行到一半时被抢占，或由于断电、系统崩溃而中断。
各种看似常规的操作都可能造成数据不一致，因此事务是必须的。
...
为了更加方便地展开描述上面的两种方法，我们先了解一下什么是事务调度。

事务调度是指一组事务的基本步骤（读、写、加锁、解锁等控制操作）的一种执行顺序。
表达事务调度的一种模型可以定义为：
...
冲突是指调度中的一对连续动作，它们满足：如果交换顺序，那么涉及的事务中至少有一个的行为会改变。
因此，有冲突的两个操作不能交换次序，而没有冲突的两个操作可以交换次序。

冲突可串行性则是指：一个调度如果能够通过交换相邻两个无冲突的操作转换成某个串行调度，则称此调度为冲突可串行化的。
...
- 共享锁 `S`：所有事务都可以读，但任何事务都不能写。
- 更新锁 `U`：初始读，以后可以升级为写。
- 增量锁 `I`：用于增量更新（例如 `A = A + x`），以区分增量更新和其他类型的更新。

有了以上几把锁，我们可以选择合适的时机加锁和解锁，因此出现了以下几个等级的封锁协议：

```{figure} ../_static/images/db_lock_grade.*

```

有了这几个等级的协议之后，程序员可以自主选择 SQL 的隔离级别（应该可以配置）：

- 读未提交，相当于 0 级封锁协议：对有写要求的数据对象 A 加排他锁，不再访问后即刻解锁。
- 读已提交，相当于 1 级封锁协议：对有写要求的数据对象 A 加排他锁，事务提交时刻解锁。
- 可重复读，相当于 2 级封锁协议：对有写要求的数据对象 A 加排他锁，事务提交时刻解锁。
  有读要求的数据对象 B 加共享锁，不再访问后即刻解锁。
- 可串行化，相当于 3 级封锁协议：对有写要求的数据对象 A 加排他锁，事务提交时刻解锁。
...
两阶段锁中的“两阶段”是指加锁阶段和解锁阶段：加锁阶段不能有解锁操作，解锁阶段不能有加锁操作。
读写数据之前要先获得锁，每个事务中所有的加锁请求都先于任何解锁请求。

两阶段锁协议是可以保证冲突可串行化的，但是可能产生死锁。

## 数据库转储

如果已经在 Windows 上安装了 MySQL 数据库，在安装路径（如 `D:\Program Files\mysql-5.7.37-winx64`）
的 `data` 文件夹下：

- 如果使用 MyISAM 引擎，可以找到 `.frm`（表结构）、`.MYD`（数据文件）、`.MYI`（索引文件）文件。
- 如果使用 InnoDB 引擎，可以找到 `.frm` 文件和 `ibdata1` 表空间文件。

因此，一种转储方式就是直接复制这些文件，另一种方式是使用 `mysqldump` 命令。
