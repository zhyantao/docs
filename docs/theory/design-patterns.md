...
当基类的析构函数被声明为 `virtual` 时，通过基类指针或引用删除派生类对象，C++ 会根据实际对象类型调用对应的析构函数，确保析构顺序为：
...
学习设计模式一定要和具体场景联系起来，知道什么时候该用哪种设计模式，才是最重要的。

在线运行代码的网址：<https://coliru.stacked-crooked.com/>
