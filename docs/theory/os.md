...
操作系统是运行于硬件之上的一段程序，属于系统软件。
它负责协调硬件、软件等计算机资源的工作，为上层用户和应用程序提供简单易用的服务。
因此，操作系统是一个资源管理者，管理者 CPU、存储器、文件与设备等资源。
...
- 异步：指进程以不可预知的速度向前推进，无法预知进程何时开始、暂停和结束

没有并发和共享，就谈不上虚拟和异步，因此并发和共享是操作系统最基本的两个特征。

### 操作系统的运行机制

用高级语言（如 C 语言）手工编写的代码，经编译器编译后可以生成硬件能够理解的机器指令。
将机器指令（代码）放到磁盘的合适位置后，处理器便从该位置开始执行。

同一份操作系统源代码中，并非所有语句都具备相同的权限。
能够直接和硬件打交道的代码是内核程序，其所执行的指令称为特权指令；
而通过内核程序提供的接口编写的程序是应用程序，执行的是非特权指令。
...
从内核态到用户态的转变，需要修改 PSW（程序状态字）中的模式位；而从用户态转变到内核态则由中断引起，由硬件自动完成。
...
CPU 在执行指令时会自动检查是否有异常发生，异常由程序自动触发。
在每个指令周期的末尾，CPU 都会检查是否有外部信号需要处理，以此判断是否需要执行外中断。
内中断和外中断都会通过中断向量表找到相应的中断处理程序。

当需要进行设备管理、文件管理、进程管理、进程通信、内存管理时，就需要使用系统调用。
系统调用是操作系统对应用程序或程序员提供的接口。
系统调用通过传参、陷入（trap）或访管指令触发，由操作系统内核程序处理请求，并将结果返回给应用程序。

宏内核性能高，但内核代码庞大、结构复杂，难以维护；微内核功能少，但需要频繁地进行状态切换，性能较差。
**在后续的实验中，我们使用的是宏内核设计。**
...
- 进程控制和管理信息：CPU、磁盘、网络流量使用情况、进程当前状态（就绪、阻塞、运行等）
- 资源分配清单：正在使用哪些文件、正在使用哪些内存区域、正在使用哪些 I/O 设备
- CPU 相关信息：如 PSW、PC 等各种寄存器的值（用于实现进程切换）

当我们通过 fork 创建一个新进程后，新进程与父进程的程序段相同，但 PCB 和数据段并不相同。
...
但一个管道只能实现半双工通信，要实现双向同时通信，需要建立两个管道。
各个进程互斥地访问管道。

3、消息传递

传递结构化的消息（包括消息头和消息体），提供“发送/接收”原语。
具体实现上，一种是直接通信：将消息直接挂到接收方的消息队列中；另一种是信箱方式：先把消息发送到信箱中。

### 线程的概念

进程是资源分配的基本单位，线程是 CPU 调度的基本单位。

同一进程内的各个线程共享该进程拥有的资源，同一进程内的线程切换不会导致进程切换。

引入线程的目的是增加并发度，减少并发带来的系统开销。

### 资源调度

所谓调度，需要回答的核心问题是：在何时将何种任务分配给何种资源以何种方式执行。
调度广泛存在于操作系统、编程语言运行时、容器编排和业务系统中，其核心目的是
“对有限的资源进行分配，以实现资源利用率的最大化、降低系统的尾延迟或最小化任务的完工时间” [^cite_ref-1]。

在计算机科学中，调度就是一种将任务（Work）分配给资源的方法。
任务可能是虚拟的计算任务，例如线程、进程或数据流；这些任务会被调度到硬件资源（如 CPU 等设备）上执行
[^cite_ref-2]。

在 Linux 操作系统中，被调度的基本单位是线程，而可分配的资源就是 CPU 时间。
...
按照某种算法从就绪队列中选择一个进程，将 CPU 分配给它。

```{note}
存储器的种类很多，按其用途可分为主存储器（又叫“内存储器”，简称“内存”）和辅助存储器（又称
“外存储器”，简称“外存”）。外存简单来说就是日常所说的“存储”，主要分为固态硬盘和机械硬盘。
内存是可以进行高速读写的存储器，包括常见的内存条、显卡内存（又称“显存”）。
```

为减轻系统负担、提高系统资源利用率，可将暂时不执行的进程调入外存，使其进入“挂起态”。
...
```{note}
“忙等”是指当前进程无法获得全部所需资源，却又在占用 CPU 的同时等待资源的一种状态。
需要特别注意的是，“忙等”并不是“忙则等待”的简称。
```

- 空闲让进：当临界区空闲时，应允许一个请求进入临界区的进程立即进入
- 忙则等待：当已有进程进入临界区时，其他试图进入临界区的进程必须等待
- 有限等待：对请求访问的进程，应保证其能在有限时间内进入临界区（防止饥饿）
- 让权等待：当进程暂时无法进入临界区时，应立即释放处理机，避免忙等待
...
在进入区只做“检查”，不
“上锁”；在退出区把临界区的使用权转交给另一个进程（相当于在退出区既为另一个进程
“解锁”，又给自己“上锁”）。
...
由于检查和上锁两个操作不是原子操作，因此不遵循“忙则等待”原则。
...
解决了“忙则等待”问题，但违背了“空闲让进”和“有限等待”原则，有可能产生“饥饿”。
...
简单、高效，但不适用于多处理机，且只适用于操作系统内核进程，不适用于用户进程。
...
可以用一个信号量表示系统中某种资源的数量，例如：系统中只有一台打印机，就可以设置一个初值为 1 的信号量。
...
注意理解时的顺序：动词始终放在前面，否则容易引发理解偏差。

```

用户进程可以通过使用操作系统提供的 `wait(S)` 和 `signal(S)`
原语来对信号量进行操作，从而方便地实现进程互斥和同步。
原语是由关中断/开中断指令实现的。
...
当资源不可用时，进程会自动进入阻塞队列，符合“让权等待”原则，不会出现“忙等”现象。
...
单生产者-单消费者问题指的是生产者生产、消费者消费的是同一种产品；
...
```{note}
可以注意到：有向图中有几条有向线段，我们就声明几个同步信号量。

如果缓冲区的大小为 1，那么可能不需要设置互斥信号量，也能实现互斥地访问缓冲区。
当然，这不是绝对的，要具体问题具体分析。
```
...
```{code-block} c
// semaphore mutex = 1;  // 缓冲区只有一个，此处省略
...
只要有读进程还在读，写进程就要一直阻塞等待，可能“饿死”。因此，在这种算法中，读进程是优先的。
...
5 位哲学家共享 5 根筷子，如何在避免死锁的情况下顺利进餐。

```{note}
在并发的情况下，如果 5 位哲学家都拿起了左边的筷子，又都在等待右手的筷子，就发生了死锁。

这些进程之间只存在互斥关系，但与之前不同，每个进程都要同时持有两个临界资源，因此就有可能发生“死锁”。

因此，如果遇到一个进程需要同时持有多个临界资源的情况，就应该参考哲学家问题的思想。
```
...
- 最多允许 4 位哲学家同时拿筷子，这样就一定能保证至少有一位哲学家可以拿起两边的筷子。
...
在并发环境下，各进程因竞争资源而互相等待对方持有的资源，导致所有进程都阻塞、都无法向前推进的现象，称为
“死锁”。发生死锁后若无外力干涉，这些进程将一直无法向前推进。
...
不同点：死锁一定是“循环等待对方手里的资源”导致的，且至少涉及两个进程。
发生死锁的进程一定处于阻塞态。
发生“饥饿”时可能只有一个进程，该进程既可能处于阻塞态，也可能处于就绪态。
发生死循环时可能只有一个进程，该进程可以处于运行态。
...
- 互斥条件：资源一次只能被一个进程使用，可共享的资源不会导致死锁。
- 不剥夺条件：进程当前占有的资源不可被剥夺，只能主动释放。
- 请求和保持条件：进程已保持一个资源，又提出了新的资源请求。
...
破坏产生死锁的四个必要条件中的一个或几个，可能会导致系统资源利用率和系统吞吐量降低。
...
```{note}
如果能够找到一个安全序列，系统就处于安全状态；如果一个都找不到，即为不安全状态。
...
如果会进入不安全状态，就暂时不批准这次请求，让该进程先阻塞等待。

##### 检测和解除（允许死锁发生）

检测死锁的算法基于资源分配图。
在资源分配图中，找出既不阻塞又不孤立的结点，消除与它相邻的所有请求边和分配边，使之成为孤立结点。
如果最后所有结点都成为孤立结点，那么这个图就是可完全简化的，得到的序列就是一个安全序列。
如果最终不能消除所有边，那么就发生了死锁，仍被边连着的结点就是处于死锁状态的结点。
...
内存是计算机中唯一既能被 CPU 直接访问、又能与外存交换数据的资源。内存管理的目标：

- 让多个进程能同时驻留内存、互不干扰（空间隔离）；
- 把逻辑地址转换成物理地址（地址转换）；
- 在小内存上运行大程序（虚拟内存）。

### 程序装入内存的三种方式

编译后形成的逻辑地址（相对地址）需要在装入内存时转换为物理地址（绝对地址）：
...
* - 静态重定位
  - 装入时
  - 装入后地址不再变化，程序运行期间不能在内存中移动
* - 动态重定位
  - 运行时（借助重定位寄存器）
  - 程序可在内存中移动，现代操作系统均采用此方式
```
...
- **单一连续分配**：整个内存只有一道程序，最简单；
- **固定分区分配**：内存预先划分为若干个大小固定的分区，每个分区装入一道程序，会产生**内部碎片**；
- **动态分区分配**：按进程实际需要分配大小可变的分区，会产生**外部碎片**，可用**紧凑**（拼接）技术解决。
...
允许进程分散装入不相邻的内存区域，是虚拟内存的基础。

#### 基本分页存储管理

把内存空间划分为大小相等的小块（**页框/页帧**），把进程的逻辑地址空间也划分为相同大小的页，
以**页表**记录页号到页框号的映射。逻辑地址结构：

```
页号 P | 页内偏移量 W
```

地址转换：页号 → 查页表 → 得页框号 → 物理地址 = 页框起始地址 + 页内偏移量。

- 优点：无外部碎片，内存利用率高，管理简单；
- 缺点：页表占用额外内存，地址转换需要硬件（快表 TLB）加速。

每个进程一张页表。**页表寄存器（PTR）**存放页表在内存中的起始地址和页表长度。
CPU 每次访问数据实际上需要**两次访存**（一次读页表、一次读数据）；引入**快表（TLB，相联存储器）**
缓存最近使用的页表项后，命中时只需一次访存。

#### 基本分段存储管理

按程序的逻辑结构（代码段、数据段、栈段）分段，每段长度可变。段表记录每段的段长与基址：

```
段号 S | 段内偏移量 W
```

- 优点：符合程序员的逻辑视角，易于实现段的共享和保护；
- 缺点：存在外部碎片，需要紧凑。

#### 段页式管理

先按逻辑结构分段，段内再分页：逻辑地址为 `段号 | 页号 | 页内偏移量`，段表指向页表。
兼具分段逻辑清晰与分页无外部碎片的优点，但需要三次访存（段表、页表、数据）。

```{note}
分页是系统视角的划分（对用户透明），页面大小固定；分段是用户视角的划分（对用户可见），段长可变。
分页的主要目的是提高内存利用率，分段的主要目的是满足逻辑模块化的需要。
```

### 虚拟内存

基于**局部性原理**（时间局部性：刚访问过的信息很快会再被访问；空间局部性：与刚访问过的信息相邻的信息很快会被访问），
操作系统把进程的一部分页先装入内存、其余部分留在外存。程序访问的页面不在内存时产生**缺页中断**，
由操作系统把该页换入内存。虚拟内存的三个主要特征：

- **多次性**：程序可分多次装入内存，而不必一次全部装入；
- **对换性**：程序运行期间允许在内存与外存之间换入、换出；
- **虚拟性**：逻辑上扩充了内存容量，地址空间不受物理内存限制。

#### 页面置换算法

内存没有空闲页框时，需要选择一个页换出到外存：
...
* - 最近最久未使用（LRU）
  ...
  - 性能接近 OPT，实现开销大
* - 时钟（CLOCK/NRU）
  - 循环扫描，访问位为 0 的页淘汰；为 1 则清零并跳过
  - 性能接近 LRU，实现开销小，实际系统常用
* - 改进型时钟
  - 同时考虑访问位与修改位，优先淘汰“未访问且未修改”的页
  - 减少写回外存的开销
```
...
- 文件如何存储在外存上（物理结构）
- 如何管理磁盘的空闲块（存储空间管理）
- 提供哪些接口（`create`、`delete`、`open`、`close`、`read`、`write`）
...
空闲表法：建立如下所示的数据结构，可采用首次适应、最佳适应、最坏适应等算法。
...
空闲链表法：如果以盘块为单位组成一条空闲链，可以称为空闲盘块链；如果以盘区为单位组成一条链，可以称为空闲盘区链。
分配空间时，从链头摘下若干空闲盘块，并修改空闲链的链头指针即可（链头指针由操作系统维护）。
回收的盘块重新添加到链表末尾即可。

位示图法：一般用若干连续的字（一个字等于 16 位）来表示磁盘中的盘块。每个二进制位对应一个盘块。

成组链接法：这是 UNIX 系统中采用的方法。在文件卷的目录区中专门用一个磁盘块作为“超级块”，系统启动时将超级块读入内存，
并保证内存与外存中的超级块数据一致。

### 磁盘调度算法

磁盘调度算法研究如何调整磁道的访问顺序，以最大限度地减少时间开销。
...
* - 寻道策略
  - 注解
* - 先来先服务（FCFS）
  - 按请求到达的先后顺序依次响应：哪个进程先发出访问请求，磁头就先移动到它要访问的磁道
* - 最短寻道时间（SSTF）
  - 在若干待处理的访问请求中，优先访问与当前磁头最近的磁道，可能产生“饥饿”现象
* - 扫描算法（SCAN）
  - 磁头先向磁道的一端移动，到达最边缘后再反向移动，因此响应请求的顺序与请求到达的顺序不一致，但解决了 SSTF 的饥饿问题
* - LOOK 调度算法
  - SCAN 需要移动到最边缘才会折返，这可能导致无效访问；LOOK 算法边移动边观察，因此不需要移动到最边缘才折返
* - 循环扫描算法（C-SCAN）
  - 磁头沿一个方向移动并响应请求，到达最外边缘后立即返回最内侧（返回途中不响应），再次从内向外移动时才响应请求
* - C-LOOK 调度算法
  - 与 LOOK 算法类似，也不需要移动到最边缘
```

访问磁盘的过程中，很大一部分时间都花费在寻道上，那么有哪些策略可以节省时间呢？
主要考虑：存取磁盘数据时，CPU 处理数据也需要时间，因此磁盘高速旋转过程中传输的数据并不一定马上就能使用。
常用的策略有下面两种：
...
* - 磁盘设计策略
  - 注解
* - 交替编号
  - 逻辑上相邻的扇区，物理上有一定的间隔。间隔旋转的这段时间最好等于处理时间
* - 错位命名
...
并且在数据传输过程中，不再需要 CPU 干预，内存和设备直接交互，只有在一个或多个数据块传输开始和结束时，才需要 CPU 干预。
...
引入脱机技术，可以缓解 CPU 与 I/O 设备之间的速度不匹配问题：先将慢速设备的数据缓存到快速设备，然后由快速设备与 CPU 交互。
“假脱机”不是硬件实现，而是一种软件实现：它将设备的输入数据先暂存到输入缓冲区（位于内存中），然后送到“输入井”（位于磁盘中）；
输出则是相反的过程：由输出井到输出缓冲区，再到输出设备。

假脱机技术的具体应用：共享打印机。
当多个用户进程提出打印请求时，系统都会响应，但并不是真正把打印机分配给它们，而是由假脱机管理进程完成下面两件事：
