...
```{note}
本实验来自 MIT 6.828（2018 版）课程，基于 x86 架构的 JOS 教学内核。
而本目录下的 [实验三](lab3)、[基本概念](concepts) 和 [内核调用图](callgraph) 来自 6.S081（2020 版）课程，
基于 RISC-V 架构的 xv6 内核，二者属于不同课程、不同架构，注意区分。
```
...
- 第一部分聚焦于熟悉 x86 汇编语言、QEMU 仿真器以及计算机的上电启动过程。
- 第二部分着重于跟踪调试 `lab/boot` 目录下的 bootloader。
- 第三部分深入 kernel 初始化过程，侧重于跟踪调试 `lab/kern` 目录下的 kernel，也就是 JOS。

```{hint}
实验结束后，你可以通过命令 `make grade` 检查作业是否做对了。
你也可以现在敲一下这个命令，看看是什么效果，方便后面对比。
```

## 准备工作环境

MIT 6.828 提到使用 Athena machine 会更加方便，但不是该学校的学生可能接触不到这个机器。
因此后续工作将在自己的虚拟机上完成，所需环境如下：
...
来安装一下；如果有，我们直接为它建立软连接或硬链接就可以了。

```{code-block} text
sudo ln -s /usr/bin/python2 /usr/bin/python
```

我们并不是完全从 0 开始写代码，而是在已有内核上添加一些新功能，因此先将内核代码克隆到虚拟机上：
...
接下来需要安装硬件仿真器 QEMU，但 6.828 并不推荐使用 qemu.org 提供的 QEMU，因此改用
6.828 打过补丁的 QEMU。但若直接使用 6.828 patched QEMU，在 `make` 时会出现一些报错。
...
上面中括号部分是可选项：若不指定 `--prefix`，则默认安装到 `/usr/local` 下；`--target-list` 会对
QEMU 进行瘦身，只编译安装指定的架构；若你不熟悉自己的 CPU 架构，可以省略这个参数 [^cite_ref-2]。
...
用的是支持 Intel 语法的 Netwide Assembler（NASM），而 6.828 用的是支持 AT&T
语法的 GNU Assembler。这意味着要学会这两种语法之间的相互转换，因此 6.828 强烈建议我们阅读
...
不知道你注意到没有，QEMU 在实验中充当的是一个虚拟裸机的角色。
这台虚拟裸机和我们在现实中用到的电脑在功能上没有什么两样，只不过这台虚拟裸机以代码的形式存在。

在真实物理机上启动操作系统并不是难事，但如果想要调试这个操作系统，问题就来了。
如何在程序启动的过程中设置断点？
这似乎是电脑上电后我们无法控制的事情；但如果这台电脑以软件的形式存在，设置断点就很好办了。

QEMU 因此诞生，它的职责就是充当一台台式机或笔记本电脑，允许我们把写好的操作系统运行在其中。

那么 QEMU 是如何起作用的呢？
其实，我们写的代码经过 QEMU 封装之后，最终仍然运行在真实物理机上。
...
那么我们现在已经有了“内核”源代码，如何将它存放到磁盘（disk）中并加载运行呢？
这个工作已经被 QEMU 完成了，它帮我们存储并解析了 bootloader（`obj/boot/boot`）和 kernel
（`obj/kern/kernel`），但它是如何存储和解析的，这些细节暂时还不清楚，下一节将会解答。
现在能知道的是，使用 `make qemu` 或 `make qemu-nox`（使用 SSH 时推荐后一种）
...
拷贝到磁盘的引导区中，插到真实物理机上，通电开机，将看到与 QEMU 仿真器输出一样的内容。
...
如果想要在金属裸机上直接运行内核源码，那么势必需要知道**内核源码的入口地址**在哪里。
如果我们不加约束，每个人都自定义入口地址，那么我写的操作系统在你的机器上可能就找不到入口地址。
...
从上面的内存地址编码可以看出，每个地址都是 32 位的，因此这是一个 32 位机。
我们还可以发现，这个机器可以寻址 4GB 的物理内存，也就是说，如果给它挂载一个 8GB
的磁盘，它将无法充分利用这块外加磁盘。
RAM 之下就是我们常说的内存，所有需要在 CPU 上运行的程序，都会先加载到 RAM 中。
图中的 Low Memory 通常被用于存储中断向量表、BIOS 控制信息、应用程序内部信息 [^cite_ref-3]。

从 `0x000A0000` 到 `0x000FFFFF` 的 384KB 是为硬件预留的区域，本例中用作图像缓冲（VGA Display）。
当然，其中最重要的用途还是存储 BIOS，它占用了 64KB 的空间。
在早期计算机中，BIOS 存储在只读存储器（ROM）中，而现在的计算机把 BIOS 存储在 flash 中。
BIOS 的作用是提供系统初始化功能，比如激活显卡、检查已用内存等。
...
这种转变发生在英特尔发布 80286 和 80386 处理器之后，突破了 1MB 的内存限制，开始支持 16MB 和 4GB
的物理地址空间。
...
由于设计的限制，JOS 仅使用前 256MB 的物理地址空间，所以在后面的实验中，我们将会假设所有计算机仅有
32 位物理地址空间。

## 内核的启动流程

知道了内核源码的入口地址，接下来就需要尝试如何启动内核程序了。
...
命令 `make qemu-gdb` 会启动 QEMU，并让内核停在第一条代码之前，方便我们一步步调试内核源码。

命令 `make gdb` 可以让我们在之前卡住的地方继续向下执行，并把执行的指令在这里显示出来。
...
之所以有上面的输出，是因为我们在 `.gdbinit` 文件中定义了某些监听规则。
从效果上观察，`make qemu-gdb` 可以让 QEMU 卡在启动过程，`make gdb` 让 QEMU
...
上面那行代码是 GDB 对内核启动后第一条指令的反汇编。从输出可以推断出一些信息：

- 计算机启动后能够找到的**内核入口地址**是 `[CS:IP]=[0xf000:0xfff0]`；
...
但现在你并不需要明白所有细节，对 BIOS 是如何启动的，有一个大致的思路就可以了。
```

完整的启动流程如下：

- 系统上电，处理器复位（重置 `CS:IP`），找到 BIOS 入口地址，执行第一条语句；
...
- 开机引导程序（bootloader）负责启动操作系统内核，系统启动成功。
...
需要注意的是，启动扇区载入内存后的物理地址并不是一成不变的，只要遵守约定，能让 BIOS
跳转到那里就可以了。
...
对于 6.828 这门课程，我们使用的是传统的磁盘启动方式，也就是说 bootloader 必须调整为 512 字节。

bootloader 包括一个汇编源文件 `boot/boot.S` 和一个 C 源文件 `boot/main.c`。
仔细阅读源文件，并确保你能看懂其中的代码，理解启动过程中发生了什么。
...
后由 GNUmakefile 创建的反汇编文件。
这个反汇编文件让我们能够更简单地看懂物理内存上到底存放了怎样的 bootloader 源代码。
...
命令 `x/i` 是一个反汇编指令，也可以用来查看内存中的值，从输出上看，内存中存的可能是指令也可能是数据。
...
在内存的 section 中，`.bss` 后紧跟着 `.data`。

因为 C 语言要求未初始化的全局变量以 0 值代替，所以我们不需要在 ELF 二进制文件中保存 `.bss` 的内容。
只需保存 `.bss` section 的位置和大小，然后让 bootloader 或源码本身给
`.bss` section 赋予 0 值即可。

如何得到内核中所有 section 的名字、大小和 link address 呢？只需要键入下面的命令即可：

```{code-block} text
objdump -h obj/kern/kernel
```

你将会看到更多 section 被列了出来，但其他部分对我们理解原理而言不是那么重要。
...
如果给出了错误的 link address，那么二进制将无法起作用。
在现代的机器中，通常使用相对地址来表示 link address，优点是易于扩展，缺点是复杂性高、性能差。
我们在本课程中不用相对路径，而是使用更简单的绝对地址来表示 link address。

一般来说，link address 和 load address 是相同的，比如看一下 bootloader 的 `.text` 部分：
...
因为 program header 中明确了 ELF 对象的哪一部分需要加载进内存，以及每一个需要使用的目标地址。
你可以通过下面的命令观察一下 program header：

```{code-block} text
objdump -x obj/kern/kernel
```

稍微解释一下上面这条命令的输出：ELF 对象把需要加载进内存的部分标记为 LOAD。
vaddr 表示虚拟地址，paddr 表示物理地址，memsz 表示加载区域的大小，filesz 表示文件大小。

在 `boot/main.c` 中，`ph->p_pa` 保存的是每一个 program header 的目标段（segment）的物理地址。
尽管 ELF 规范对该字段的实际含义表述模糊，它实际上是一个物理地址。

BIOS 从 `0x7c00` 将启动扇区载入内存，这是 bootloader 的 load address，也是 link address，
因此也可以将 `0x7c00` 称为 bootloader 的入口地址。
我们可以通过给 `boot/Makefrag` 中的 linker 传递参数 `-Ttext 0x7c00` 来设置 link address，让
linker 可以在生成的代码中产生正确的内存地址。

```{admonition} 练习 5
重新追踪 bootloader 最初的几条指令，确定程序从哪里开始 break；或者说，如果你拿到了
错误的 linker address，bootloader 从哪里开始报错。然后尝试在 `boot/Makefrag` 中更改错误的
link address，运行 `make clean` 重新编译实验代码（`make`），再重新追踪 bootloader，观察发生了什么。

答案：由实模式转换为保护模式失败了。原因是全局符号描述表出现了问题 [^cite_ref-6]。
```

对比 kernel 和 bootloader 的 load address / link address：
在 bootloader 中这两个地址是相同的，但 kernel 中这两个地址不同。
这是因为 bootloader 会把 kernel 加载到低地址（1MB），却让 kernel 从高地址开始执行。
我们将会在下一节深入探讨如何在这种思路下实现内核代码。

除了 section 信息，ELF header 中还有一个对我们来讲重要的字段：`e_entry`。
这个字段保存了 entry point 的 link address，即程序 `.text` section
...
根据 load address 把每个 section 从磁盘读取到内存中，然后跳转到 kernel 的
entry point，按照从高地址往下的顺序依次执行指令。

```{admonition} 练习 6
参考 [[webpage](https://sourceware.org/gdb/current/onlinedocs/gdb/Memory.html)]，使用命令
`x/Nx ADDR` 检查内存，它会打印内存从 `ADDR` 开始的 `N` 个字。
需要注意的是，字的大小并不是统一标准：在 GNU 汇编中，一个字是 2 个字节（16 bit）。
...
为什么会产生这种变化？因为 `bootmain` 函数在最后会把内核的各个程序段送入内存地址 `0x00100000`
处，所以这里存放的就是内核的某一个段的内容；由于程序入口地址是 `0x0010000C`，正好位于这个段中，
所以可以推测这里面存放的应该是指令段，即 `.text` 段的内容 [^cite_ref-6]。
```

## 分析 kernel 源码

我们在这一小节更详细地了解 minimal JOS kernel：

- 内核是如何完成从虚拟地址到物理地址的映射的？
- 如何将内核运行时的信息更规范地输出到屏幕上？

和 bootloader 一样，kernel 也是以一些汇编代码开始，完成必要的初始化后，C 代码才可以正常执行。

使用虚拟内存来解决位置依赖性。
前面讲到 bootloader 的 link address 和 load address 是一样的，但 kernel 的这两个地址并不相同。
这是因为链接 kernel 比链接 bootloader 更复杂，所以 link address
和 load address 位于 `kern/kernel.ld` 的最顶端。

OS kernel 经常在非常高的虚拟地址（virtual address）上进行链接和运行，比如
`0xf0100000`，这是为了把处理器的低虚拟地址空间留给用户程序使用。

但许多机器并没有物理地址 `0xf0100000`，所以我们不能指望把内核放在那个位置。
...
- **线性地址** 指虚拟地址到物理地址变换的中间层，是 _处理器可寻址的内存空间中的地址_。
  ...
  如果开启了分页机制，那么线性地址需要再经过变换，转换为物理地址。
  如果无分页机制，那么线性地址就是物理地址。
- **物理地址** 指 CPU 外部地址总线上寻址物理内存的地址信号，是地址变换的最终结果。

只有在 `kern/entry.S` 设置了 `CR0_PG` 标志后，内存引用才会被视为“物理地址”。
更严格地说，这里说的内存引用实际上是线性地址：因为在 `boot/boot.S`
中设置了从线性地址到物理地址的线性映射，所以才能将这个线性地址看作物理地址。
所以说，一旦 `CR0_PG` 被设置，内存引用就会经过虚拟内存硬件的翻译，从虚拟地址变为物理地址。
而充当翻译角色的代码是 `kern/entrypgdir.c`。它把虚拟地址 `0xf0000000`-`0xf0400000`
翻译为物理地址 `0x00000000`-`0x00400000`，而虚拟地址 `0x00000000`-`0x00400000` 也会被翻译为
`0x00000000`-`0x00400000`。

任何不在这个范围的虚拟地址都会导致硬件异常，但我们目前还没有设置中断处理机制，这会导致 QEMU
转储机器状态并退出；如果你没有使用 [6.828 patched QEMU](https://github.com/mit-pdos/6.828-qemu)
的话，则会永无止境地重启。

```{admonition} 练习 7
使用 `qemu-gdb` 追踪 JOS kernel，并在 `movl %eax, %cr0` 停止，检查内存 `0x00100000` 和 `0xf0100000`
中的内容。然后，使用 `stepi` 单步调试，重复检查 `0x00100000` 和 `0xf0100000` 中的内容。
确保你能理解发生了什么。

在新映射建立之后，第一条由于映射关系的错误发生失败的指令是什么？
在 `kern/entry.S` 中注释掉 `movl %eax, %cr0`，追踪这个错误映射，检验你的观察是否正确。
```

下面我们将介绍如何格式化输出到 Terminal。在 OS kernel 中，我们将会自己实现所有的 I/O，包括
`printf()`。

阅读 `kern/printf.c`、`lib/printfmt.c`、`kern/console.c`，并确保你能理解它们之间的关系。
...
3. 下面的问题你可能需要参考 lecture 2 的笔记，这些笔记涵盖了 x86 平台上的 GCC 调用规则。
   ...
   - 对于 `cons_putc` 列出它的参数；对于 `var_arg`，分别列出调用前后 `ap` 指向了什么。
...
6. 假设 GCC 更改了调用规则，按照声明的顺序将参数压栈，那么最后一个声明的参数是最后一个压栈的。
   你将如何更改 `cprintf` 或它的接口，让它能够接收可变数量的参数？

```{admonition} 挑战
加强版的 console 可以在 Terminal 中输出不同颜色的字体。
传统的做法是用 ANSI escape sequence 包裹文本，然后由 Terminal 来解释这段包裹代码。
如果你有兴趣，可以参考 [[webpage](https://pdos.csail.mit.edu/6.828/2018/reference.html)]
根据自己的兴趣做更改，这可能涉及 VGA 显示硬件的编程。
...
```{admonition} 练习 9
kernel 执行到什么语句时开始创建栈空间？并确定栈空间保存在内存的什么位置。
kernel 是如何为它的栈预留空间的？栈指针指向的是栈顶还是栈底？
...
从栈中弹出一个值，包含读取栈指针指向的值、栈指针增加一个单位两个过程。
许多 x86 指令，比如 `call`，都借助栈指针寄存器采用直接地址寻址的方式找到相应的指令或数据。
在 32 位模式下，栈只能存取 32 位的值，并且 esp 寄存器通常按 4 字节对齐。

按照软件惯例，ebp 寄存器（基指针）主要与栈相关联。
在 C 函数的入口处，函数的 prologue 代码将前一个函数的基指针压栈来保存它；
然后，为了保持函数之间的调用关系，把当前 esp 的值拷贝到 ebp。
如果程序中所有函数都遵守这个约定，那么在程序执行期间的任何时间点，都可以通过栈回溯找到函数之间的调用关系。
发现调用关系的能力非常重要：比如某个函数由于非法参数触发了 assert 或
panic，但你不知道这个非法参数是谁传递过来的，那么栈回溯就可以帮你应对这种场景。

```{admonition} 练习 10
熟悉 x86 平台上的 C 语言调用规则，找到 `obj/kern/kernel.asm` 中函数 `test_backtrace`
的入口地址，在那里设置一个断点，然后检查内核启动后每次调用时都会发生什么。
在每个 `test_backtrace` 级别上，有多少个 32 位的字被压栈？这些被压入的字都是什么？

注意，如果你想成功完成这个实验，你需要使用 6.828 patched
QEMU，否则需要自行把断点的内存地址翻译为线性地址。
```

练习 10 包含了关于如何实现栈回溯函数的一些信息，这些信息来源于函数 `mon_backtrace()`。
它的原型函数已经在 `kern/monitor.c` 中实现了。
如果你打算用纯 C 代码编写，你会发现 `inc/x86.h` 中的 `read_ebp()` 函数十分有用。
你可能也需要在 kernel monitor 的命令参数列表中对这个新函数创建钩子，然后它就可以与用户交互了。
...
`ebp` 表示函数正在使用的基指针，它保存于函数入口之后、创建基指针的代码（prologue code）的后面。

`eip` 是函数 return 指令的指针，即函数返回时的那条控制指令的地址。
return 指令指针通常指向 `call` 指令之后的那条指令（为什么呢）。

`args` 是函数的参数列表，在本例中它有 5 个可用参数。
这 5 个十六进制数字是函数的前 5 个参数，在函数调用发生之前就会被压栈。
...
通过学习 `kern/entry.S`，你可以发现这里有一个简单的方式告诉函数什么时候停止。

阅读 _C 语言程序设计_ 时，有一些重要的知识点需要你记忆，后面的实验可能会用到：

- 若 `int *p = (int *) 100`，则 `(int)p + 1` 和 `(int)(p+1)` 的值分别是 101 和 104。
  当给指针加上一个整型数字时（如第二种情况所示），这个整型数字会被默认乘以指针指向的对象的大小。
- `p[i]` 和 `*(p+i)` 是等价的，都是 `p` 指向的内存空间中的第 `i` 个对象
- `&p[i]` 和 `(p+i)` 是等价的，都是 `p` 指向的内存空间中的第 `i` 个对象的地址

虽然大多数 C 代码都不需要指针和整型数字之间的相互转换，但操作系统却经常需要这么做。
无论什么时候在内存地址上做加法，你都要问问自己：这是**整型加法**还是**指针加法**？
检查一下，如果是整型数字加到了指针上，是否乘了相应的倍数。
...
如果你尝试关闭优化，将会导致栈帧的重新排序。你需要亲自调试 `mon_backtrace()`
...
并返回对应内存地址上的调试信息。关于 `debuginfo_eip()` 函数的定义，参考 `kern/kdebug.c`。

````{admonition} 练习 12
在这个练习中，我们将实现对栈帧的格式化打印，充分展示函数之间的调用关系：
针对每一个 `eip`，修改它对应的函数名、源文件名、行号等栈回溯显示信息。
...
- `kern/monitor.c:143: monitor+106`：函数名：函数的第一条指令相对于 `eip`
  的偏移：函数最后一条指令相对于第一条指令的偏移（单位：字节）。

提示：你可以用类似 `printf("%.*s", length, string)` 这样的语句让输出看起来更标准。

你可能会发现在打印输出中丢失了一些栈轨迹。
比如，你可能看到了关于 `monitor()` 的调用，但没有看到 `runcmd()`，这是因为编译优化内联了一些函数调用。
其他优化可能也会导致你看到不希望看到的行号。
如果你想让栈轨迹看起来更合理，可以在 GNUmakefile 中使用 `-O2` 参数，但这会导致运行速度降低。
````
