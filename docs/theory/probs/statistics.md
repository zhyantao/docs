...
我们用直觉就可以知道，如果这个 $\alpha$ 很小，那么在原假设成立的条件下，我们几乎不可能得到现在这个结果。
...
有了这个量化指标，我们就能够判断某些数据是否服从某个分布，
或者说，判断是否有足够的理由相信这些数据服从某个分布。

P.S. 原假设一般是假定某个数据服从均值为 $x$、方差为 $y$ 的正态分布或其他分布。

在给出了显著性水平的情况下，根据原假设中数据服从的分布，我们可以**通过查表**判断上面的不等式是否成立。
...
可以使用检验统计量来确定是否要否定原假设。检验统计量将你的数据与原假设下的预期结果进行比较。
...
先看下面公式中不等式右边的取值：显著性水平 $\alpha$ 这个阈值通常是**人为设定**的，一般取 0.05。
...
以 $Z$ 检验为例，检验统计量为 $Z$ 统计量，在原假设下它服从标准正态分布，公式 {eq}`eq-002` 变为公式 {eq}`eq-003`：
...
假设你执行 $\alpha$ 为 0.05 的双尾 $Z$ 检验，[查表获取 $Z$ 统计量](http://staff.ustc.edu.cn/~jbs/applex3.pdf)
...
以便抓住问题的实质；此外，还要将试验结果的主要因子和次要因子区分开，以便集中力量研究几个主要因子。
